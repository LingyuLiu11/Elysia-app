...
class UsersLoginTest < ActionDispatch::IntegrationTest
  ...
  test "login with valid information followed by logout" do
    get sessions_login_path
    post login_sessisessions_login_pathons_path, params: { session: { email:    @user.email,
                                          password: 'password' } }
    assert is_logged_in?
    ...
    assert_select "a[href=?]", logout_sessions_path
    ...
    assert_select "a[href=?]", logout_sessions_path,      count: 0
